...
def _triple_barrier(close, high, low, events, ptSl, molecule, line):
    ...
    for loc, t1 in tqdm(events_['t1'].fillna(close.index[-1]).iteritems(), desc=f'Since {molecule[0]}', position=line, total=len(molecule), leave=False):
        ...
        df0_sl = (low[loc:t1]/close[loc]-1) if events_.at[loc, 'side'] == 1 else -(high[loc:t1]/close[loc]-1)
        df0_pt = (high[loc:t1]/close[loc]-1) if events_.at[loc, 'side'] == 1 else -(low[loc:t1]/close[loc]-1)
        # earliest stop loss.
        out.loc[loc, 'sl'] = df0_sl[df0_sl < sl[loc]].index.min()
        # earliest profit taking.
        out.loc[loc, 'pt'] = df0_pt[df0_pt > pt[loc]].index.min()
    return out
...
def triple_barrier(data, t1, side, ptSl, core=cpu_count(), min_ret=0):
    ...
    with Pool(core, initargs=(RLock(),), initializer=tqdm.set_lock) as pool:
        ...
        args = list((data['close'], data['high'], data['low'], events, ptSl, m, i)
...
if __name__ == '__main__':
    ohlcv = pd.read_csv('data/BTCSPOT_300.csv', index_col=0, parse_dates=True)
    side_labeling = triple_barrier(ohlcv, 1, 1, (3, 3))
    side_labeling.to_csv('data/side_labeling.csv')
    long_labeling = triple_barrier(ohlcv, 1, 1, (3, 1), min_ret=0.00065)
    long_labeling.to_csv('data/long_labeling.csv')
    short_labeling = triple_barrier(ohlcv, 1, -1, (3, 1), min_ret=0.00065)
    short_labeling.to_csv('data/short_labeling.csv')